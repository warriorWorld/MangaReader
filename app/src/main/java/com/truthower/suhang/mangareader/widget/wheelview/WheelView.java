...
package com.truthower.suhang.mangareader.widget.wheelview;
...
import com.truthower.suhang.mangareader.widget.wheelview.adapters.WheelViewAdapter;
...
public class WheelView extends View {
	 ...
	 */
    private int[] SHADOWS_COLORS = new int[]{0xefD2D6DB, 0xcfD2D6DB, 0x3fD2D6DB};
    ...
    // Items layout杩欎釜杞瓙鏄釜linearlayout 鎯宠鏀归棿璺濆彲浠ョ洿鎺ュ湪杩欎釜鍒濆鍖栫殑鏃跺�欐敼
    ...
    // 鐢ㄤ簡涓�涓笉澶ソ鐨勬柟娉曡В鍐硈hadow闂 鍥犱负shadow闂鏄湁itemheight鐨勫�肩殑闂瀵艰嚧鐨�
    private int itemMargin, rectHeight;
    ...
    private void initData(Context context) {
        ...
        // interval = DisplayUtil.dip2px(getContext(), 20);
        // lp.setMargins(0, interval, 0, 0);
        // lp.gravity = Gravity.CENTER_HORIZONTAL;
    }

    // public void setInterval(int interval) {
    // this.interval = DisplayUtil.dip2px(getContext(), interval);
    // lp.setMargins(0, this.interval, 0, 0);
    // }
    ...
    private int getDesiredHeight(LinearLayout layout) {
        if (layout != null && layout.getChildAt(0) != null) {
            // TODO 闂磋窛闂
            LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) itemsLayout.getChildAt(0).getLayoutParams();
            itemMargin = lp.bottomMargin + lp.topMargin;
            itemHeight = layout.getChildAt(0).getMeasuredHeight() + itemMargin;
        }
    ...
    private int getItemHeight() {
        ...
        if (itemsLayout != null && itemsLayout.getChildAt(0) != null) {
            ...
            LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) itemsLayout.getChildAt(0).getLayoutParams();
            itemMargin = lp.bottomMargin + lp.topMargin;
            itemHeight = itemsLayout.getChildAt(0).getHeight() + itemMargin;
            return itemHeight;
        }
    ...
    private void drawShadows(Canvas canvas) {
		 ...
		 * getItemHeight()); //
		 */
        int height = (int) ((getHeight() - rectHeight) / 2);
        // */
        ...
        Log.d("涓変釜", "height" + height + "getHeight() - height" + (getHeight() - height));
    }

    /**
     * Draws items
     * <p/>
     * 杩欎釜鏂规硶涓嶆槸缁樺埗鍏蜂綋item鐨� 鑰屾槸婊氬姩涓粯鍒舵暣涓疆瀛愮殑 鏄粯鍒朵竴涓姩鎬佺殑鏁堟灉
     *
     * @param canvas the canvas for drawing
     */
    private void drawItems(Canvas canvas) {
        ...
        // offset鏄釜璋冩暣鍊�,鐪嬫牱瀛愭槸涓轰簡璁﹊tem杈惧埌姝ｇ‘鐨勪綅缃�屽瓨鍦ㄧ殑
        // Log.d("wheel view", "top" + top + "scrollingOffset" +
        // scrollingOffset);
    ...
    /**
     * Draws rect for current value TODO
     */
    private void drawCenterRect(Canvas canvas) {
        int center = getHeight() / 2;
        // int offset = (int) (getItemHeight() / 2 * 1.2);
        int offset = DisplayUtil.dip2px(getContext(), 26);
        rectHeight = offset * 2;
		/*
		 * / Remarked by wulianghuan 2014-11-27 娴ｈ法鏁ら懛顏勭箒閻ㄥ嫮鏁剧痪鍖＄礉閼板奔绗夐弰顖涘伎鏉堬拷 Rect rect =
		 * new Rect(left, top, right, bottom) centerDrawable.setBounds(bounds)
        ...
        paint.setColor(getResources().getColor(R.color.wheelview_divider));
        // 鐠佸墽鐤嗙痪鍨啍
