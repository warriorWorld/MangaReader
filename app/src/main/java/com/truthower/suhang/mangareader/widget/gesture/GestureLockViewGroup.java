...
//            mMarginBetweenLockView = (int) (mGestureLockViewWidth * 0.25);
            // 计算每个GestureLockView的宽度  1/n屏宽
            mGestureLockViewWidth = (int) ((mWidth * 1.0f) / 7);
            //计算每个GestureLockView的间距
            mMarginBetweenLockView = (int) ((mWidth - mGestureLockViewWidth * mCount) * 1f / (mCount + 1));
...
//                    s = s.replaceAll("\\]", "");
    ...
    private String getChooseAnswer() {
        ...
        for (int i = 0; i < mChoose.size(); i++) {
            int j = mChoose.get(i) - 1;
            res = res + j;
        }
        return res;
    }

//    public void toggleAnswerRightAndWrongState(boolean right) {
//        if (right) {
//            mFingerUpColor = 0xFF00FF00;
//        } else {
//            mFingerUpColor = 0xFFFF0000;
//        }
//        changeItemMode();
//        invalidate();
//    }
...
//                VibratorUtil.Vibrate(context, 500);
    ...
    private boolean checkPositionInChild(View child, int x, int y) {
        ...
        int padding = (int) (mGestureLockViewWidth * 0.05);
